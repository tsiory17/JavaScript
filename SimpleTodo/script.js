...
//let s get all Td and when one is selected we gonna put the class on the parent elemet 
 ...
 let allTr = document.querySelectorAll("#table_todo tr");
...
btnAdd.addEventListener("click",function(event){
  ...
  for(tr of allTr){
    if (tr.classList.contains("clickedTr")){
...
//  console.log(datas)
  ...
  allTr = document.querySelectorAll("#table_todo tr");
...
// Remove all className from tr to avoid repetition
    ...
    //  console.log(`update button ${allTr.length} tr `)
      ...
      let trToManipulate = document.querySelector('.clickedTr')
      let tdToUpdateList = trToManipulate.children;
      console.log(tdToUpdateList.length);
      for(td of tdToUpdateList){
...
document.addEventListener("click", changeToVisible)
function changeToVisible (){
  buttons = document.querySelectorAll('button')
  console.log(buttons)
  for (btn of buttons){
    btn.classList.toggle("visibleBtn");
